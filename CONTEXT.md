...
The untrusted backend boundary that validates a closed set of frontend operational events, attaches server-owned context, and forwards accepted safe log events without trusting client identity.
_Avoid_: Frontend-to-vendor logging, client audit authority
...
**Telemetry Budget**:
The explicit sustained-volume, burst-volume, event-size, queue, and provider-outage limits that a logging pipeline must satisfy without degrading business traffic.
_Avoid_: Unlimited best-effort logging, assumed capacity
...
- Every persisted backend log, metric attribute, trace attribute, and audit record must satisfy a **Safe Log Event** contract owned by its event family.
- **Frontend Log Ingestion** accepts only frontend event families that can be converted into **Safe Log Events**; authoritative security and audit events are generated by the backend.
- `engineering-frontend` owns when a **Frontend Operational Event** is emitted; `engineering-observability` owns its safe shape, delivery, ingestion, retention, and capacity policy.
- **Frontend Log Ingestion** must satisfy its **Telemetry Budget** under normal load, event storms, malicious traffic, a full queue, and provider outages.
